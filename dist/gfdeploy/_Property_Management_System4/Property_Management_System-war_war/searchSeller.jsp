...
function loadStaffTable() {
    fetch('LoadSellerTable')
        ...
        .then(data => {
            const propertyTableBody = document.getElementById('sellerViewTableBody');
...
<main>
  <div class="container">
    <form class="form-container" action="SearchSeller" method="POST">
      <h1>Search Seller Info</h1>
      <input type="text" name="searchSeller" placeholder="Search Seller">
      <br>
   ...
   <div class="property-listing">
      ...
      <table>
        ...
        <tbody id="sellerViewTableBody" ${hideSellerTableLabel ? 'style="display:none;"' : ''}>
          <!-- Property data will be dynamically loaded here -->
                        
        </tbody>
        <tbody id="staffSearchTableBody" >
          <!-- Property data will be dynamically loaded here -->
                        <% List<Seller> filteredSeller = (List<Seller>) request.getAttribute("filteredSeller");
                           if (filteredSeller != null) {
                               for (Seller sell : filteredSeller) { %>
                                   <tr>
                                       <td><%= sell.getsName() %></td>
                                       <td><%= sell.getsEmail() %></td>
                                       <td><%= sell.getsPassword() %></td>
                                       <td><%= sell.getsAge() %></td>
                                       <td><%= sell.getsGender() %></td>
                                       <td><%= sell.getsPhoneNo() %></td>
                                       <td><%= sell.getsAddress() %></td>
                                   </tr>
                               <% }
                           } %>
        </tbody>
